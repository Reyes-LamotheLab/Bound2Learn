...
filename_tr = uigetfile('*tracksdata*.mat*','Pick prediction file');
pred_file = importdata(filename_tr);
pred_file_2 = pred_file.Segmented_Tracks;
class_bound = find(Training_classification);
num_tracks = length(class_bound);
pred_file_2 = pred_file_2(class_bound,:);
displacements=cell(num_tracks,1);
% tracks_bound=cell(num_tracks,1);
% displacements_bound=cell(num_tracks,1);
% tracks_unbound=cell(num_tracks,1);
