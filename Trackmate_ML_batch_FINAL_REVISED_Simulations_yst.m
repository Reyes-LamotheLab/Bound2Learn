%% **
function Trackmate_ML_batch_FINAL_REVISED_Simulations_yst
...
function Classifier_2
    ...
    filename_new_data = uigetfile('*NOQ*.mat','Pick new data files', 'Multiselect', 'off');
    ...
    Fraction_factors = inputdlg('Quality Factor', 'Fraction Factors', [1 100],{'200.00'});
    ...
    for i = 1:length(filename_new_data(1))

        new_data = importdata(filename_new_data);
        ...
        tracks = importdata(filename_new_data);
        ...
        filename_tracks_save = strrep(filename_new_data,'NOQ', 'Q');
...
function Concatenate_tracks 
    ...
    if conc_input{1} == 'Y'
        filenames_TR = uigetfile('*tracksdata.mat', 'Training','Multiselect', 'off');
        %filenames_TR = filenames_TR'; 
        num_files_TR = length(filenames_TR(1));
        Tracks_cell_TR = cell(num_files_TR,1);
        Tracks_cell_seg = cell(num_files_TR,1);
        for i = 1:num_files_TR

                 ld_TR = importdata(filenames_TR);
        ...
        filenames_TR = uigetfile('*NOQ*.mat', 'Training','Multiselect', 'off');
        filenames_TR = filenames_TR; 
        num_files_TR = length(filenames_TR(1));
        Tracks_cell_TR = cell(num_files_TR,1);
        Tracks_cell_seg = cell(num_files_TR,1);
...
function Classifier_1
    ...
    classifier = importdata(strcat(path_classifier,filename_classifier));
    filename_new_data = uigetfile('*tracksdata.mat','Pick new data files', 'Multiselect', 'off');
    %filename_new_data = filename_new_data';

    filename_tracks_combined = uigetfile('*tracks_training_combined*', 'Pick tracks combined file');
    Fraction_factors = inputdlg('Speed Factor', 'Fraction Factors', [1 100],{'0.78'});
    tracks_combined = importdata(filename_tracks_combined);
    ...
    for i = 1:length(filename_new_data(1))
        ...
        new_data_4 = new_data_3(pred_isolate,:);
         ...
         filename_tracks_save = strrep(filename_new_data,'tracksdata.mat', 'NOQ.mat');
...
function Training_combiner
    ...
    tracks_training_final = vertcat(Tracks_cell_TR{:});
    save('classification_combined_revised.mat', 'tracks_classification_final')
    save('training_combined_revised.mat', 'tracks_training_final');
end
...
function Training_data
    ...
    for i = 1:num_tracks
        ...
        for k = vec_read(1):vec_read(end)
         ...
         h1.Visual.setPropertyValue('DataRangeMin',0);
         h1.Visual.setPropertyValue('DataRangeMax',500);
        ...
        Training_classification(i) = str2num(user_input{1});
        close(h1)
    end
    %% 
...
function Learner_model
    ...
    if strcmpi('Combined',learn_input{1})
        filename_trainer = uigetfile('*Training*.mat','Pick training file');
...
if skip_filter_input{1} == 'N' | skip_filter_input{1} == 'n'
    ...
    filenames = uigetfile('*_Q*.mat', 'Pick the segmented tracks .mat files','Multiselect', 'off');
     %filenames = filenames';
     num_files = length(filenames(1));
     Tracks_cell = cell(num_files,1);
     training_cell = cell(num_files,1);
     for i = 1:num_files
             ld=importdata(filenames);
...
%     disp('classification tracks')
    ...
    if length (intensities_final_bound) < 50
     ...
     if BestModel_intensities.ComponentProportion(1) > 0.90 | BestModel_intensities.ComponentProportion(2) > 0.90
         On_time_bound_single = On_time_final_bound;
        intensities_bound_single = intensities_final_bound;
        tracks_bound_single =  tracks_final_bound;
     end
    ...
    filenames_tracks = uigetfile('*TrackMate_tracks_bound_single.mat', 'Pick the segmented tracks .mat files','Multiselect', 'off');
