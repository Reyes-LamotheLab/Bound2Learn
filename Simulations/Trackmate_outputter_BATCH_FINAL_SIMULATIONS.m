...
filenames_images = uigetfile('*binary*.tif', 'Pick Binary Image Files', 'Multiselect', 'off');
filenames_spots = uigetfile('*spots.csv', 'Pick spots.csv', 'Multiselect', 'off');
filenames_tracks = uigetfile('*tracks.csv', 'Pick tracks.csv', 'Multiselect', 'off');
num_files_images = length(filenames_images(1));
...
folder_save = strcat('Analysis','_','Time_Int',num2str(user_input_tracks{1}),'_',num2str(user_input_tracks{4}),'_','Trkwd_',num2str(user_input_tracks{5}),'_','datpt_',num2str(user_input_tracks{2}),'_','Spt_Thr_', num2str(user_input_tracks{6}));
mkdir(folder_save);
...
bin_image = imread(filenames_images);
[row, column, v] = find (bin_image ==1);
%% 

filename_spot = filenames_spots;
filename_track = filenames_tracks;
